...
function DatosUsuario() {
  ...
  const fetchUsuario = useCallback(() => {
    return fetch("https://youmarket-entrega5.herokuapp.com/usuario/getUser", {
      headers: {
  ...
  const fetchSuscripcion = useCallback(() => {
    return fetch("https://youmarket-entrega5.herokuapp.com/usuario/getSuscripcion", {
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json",
        Authorization: "Bearer " + localStorage.getItem("auth"),
  ...
  function deleteUser() {
    var url = "/logout";
    fetch("https://youmarket-entrega5.herokuapp.com/usuario/eliminarUsuario", {
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
        Authorization: "Bearer " + localStorage.getItem("auth"),
  ...
  const fetchDireccion = useCallback(() => {
    return fetch("https://youmarket-entrega5.herokuapp.com/direccion/principal", {
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json",
        Authorization: "Bearer " + localStorage.getItem("auth"),
  ...
  const fetchPagoSus = useCallback(() => {
    return fetch("https://youmarket-entrega5.herokuapp.com/suscripcion/pagada", {
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json",
        Authorization: "Bearer " + localStorage.getItem("auth"),
  ...
  const fetchUltimaSuscripcion = useCallback(() => {
    return fetch("https://youmarket-entrega5.herokuapp.com/factura/lastSuscripcion", {
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json",
        Authorization: "Bearer " + localStorage.getItem("auth"),
      ...
      <div className="container">
        <Card title="Información Personal" style={{ margin: 20 }}>
          <div>
            ...
            {suscripcion && suscripcion.precio && (
              <p>Precio: {suscripcion.precio.toFixed(2)} €</p>
            )}
            <p>Pedidos restantes: {usuario.pedidosRestantes}</p>
            {ultimaSuscripcion.fechaFactura ? (
              <p>
                Último pago realizado de la suscripción:{" "}
                {new Date(ultimaSuscripcion.fechaFactura).toLocaleDateString()}
              </p>
            ) : (
              <p></p>
            )}
          ...
          {!pagada && (
            <div>
              ...
              <PayPalButton
                ...
                onSuccess={() => {
                  setTimeout(() => {
                    fetch("https://youmarket-entrega5.herokuapp.com/factura/createSuscripcion", {
                      headers: {
                        "Content-Type": "application/json",
                        Accept: "application/json",
                        Authorization: "Bearer " + localStorage.getItem("auth"),
        ...
        <div className="a-cambio-perfil">
          ...
          {meses === 1 && (
            <div>
              <p>
                Ya ha realizado una modificación en su suscripción para el mes
                siguiente. Hasta entonces, no podrá volver a modificarla.
              </p>
            </div>
          )}
        </div>
        ...
        {
          <button
            className="button-perfil button-finish"
            onClick={() => {
