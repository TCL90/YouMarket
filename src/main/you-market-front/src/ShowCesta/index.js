...
function ShowCesta() {
  ...
  const fetchCesta = useCallback(() => {
    return fetch(`https://youmarket-entrega5.herokuapp.com/cesta/${id}`, {
      headers: {
  ...
  const fetchProductoCesta = useCallback(() => {
    return fetch(`https://youmarket-entrega5.herokuapp.com/cesta/productos/${id}`, {
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json",
        Authorization: "Bearer " + localStorage.getItem("auth"),
  ...
  function storeAsCarrito() {
    productoCesta.forEach((prod) => {
      storeProdSession(
        ...
        prod.producto.precioIva,
        prod.producto.urlImagen,
        prod.producto.supermercado,
  ...
  return (
    <div>
      ...
      <div className="cesta-container-show container">
        ...
        {productoCesta &&
          productoCesta.map((productoC) => (
            <div key={productoC.producto.id} className="div-productos-cesta">
              <Link to={`/show/producto/${productoC.producto.id}`}>
                {" "}
        ...
        {productoCesta && productoCesta.length > 0 ? (
          <div className="grid2-carrito-cesta">
            <button
              ...
              className="submit-cesta-carrito button-finish"
            >
